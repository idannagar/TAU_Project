...
class KYFG_FRAME_RAW_C_STYLE(KYFG_AUX_DATA_C_STYLE):
    _fields_ = [
...
#----------------------------- KYFG_FRAME_AUX_DATA_RAW -----------------------------------------#
class KYFG_FRAME_AUX_DATA_RAW:
    sequence_number = 0
    timestamp = 0
    reserved = 0

class KYFG_FRAME_AUX_DATA_RAW_C_STYLE(KYFG_AUX_DATA_C_STYLE):
    _fields_ = [
        ("sequence_number", c_uint),
        ("sequence_number", c_ulonglong),
        ("reserved", c_uint) ]
...
pBuffer_c = [0 for i in range(16)]

pBuffer_ind = 0
def pBuffer_ind_next():
    global pBuffer_ind
    if (pBuffer_ind == 15):
        pBuffer_ind = 0
    pBuffer_ind = pBuffer_ind+1
...
ERROR_STATUS = {
    ...
    0x4003: 'INVALID_FGHANDLE',
    0x4004: 'INVALID_INPUT_ARGUMENT'
}


INVALID_FGHANDLE_VAL = 4294967295
...
def return_handle(fghandle):
    if (fghandle != INVALID_FGHANDLE_VAL):
...
def KYFG_Init(path: 'Default' = "KYFGLib_vc141.dll"):
    global kydll
    if ( isinstance(path,str) == False ):
        return return_status(INPUT_ARGUMENT_TYPE_ERROR) 
    if platform.system() == "Windows":
        kydll = ctypes.CDLL(path)
    else:
        kydll = ctypes.CDLL("/opt/KAYA_Instruments/lib/libKYFGLib.so")
...
def KYFG_Open(grabberIndex):
    ...
    if (connected != INVALID_FGHANDLE_VAL):
        return (FGHANDLE(connected),)
    if (exceptions_enabled() == True):
        raise KYException(ERROR_STATUS[INVALID_FGHANDLE])
    # If there is an errror and Exceptions Disabled
    return (FGHANDLE(INVALID_FGHANDLE),)
...
def KYFG_OpenEx(index, projectFile):
    ...
    if (OpenEx_status != INVALID_FGHANDLE_VAL):
...
@params:
    ...
    pBuffer - Empty list (memory allocated by user) of BufferSize size. Type: byte list
              Currently nBufferSize parameter MUST be equal to size returned by KYFG_StreamGetInfo() with info command KY_STREAM_INFO_PAYLOAD_SIZE
...
#         return return_status(INPUT_ARGUMENT_TYPE_ERROR)
    ...
    #print("INT_DEBUG_BUF_SIZE: " + str(len(pBuffer)))
    
    
    pBuffer_c_t = aligned_array(4096, c_ubyte, 307200)
#    print("Address of alligned array: " + hex(addressof(pBuffer_c_t)))
    pBuffer_c[pBuffer_ind] = pBuffer_c_t
    pBuffer_ind_next()
    
    pBuffer_c_p = pointer(pBuffer_c_t)
    
    KYFG_BufferAnnounce_status = kydll.KYFG_BufferAnnounce(streamHandle, pBuffer_c_p, len(pBuffer), pPrivate, byref(pBufferHandle_c))
...
def KYFG_BufferGetInfo(streamBufferHandle, cmdStreamBufferInfo):
    ...
    if (pInfoType.value == KY_DATA_TYPE.KY_DATATYPE_SIZET):
        ...
        KYFG_BufferGetInfo_status = kydll.KYFG_BufferGetInfo(streamBufferHandle, cmdStreamBufferInfo, pInfoBuffer, None, None)
    else:
        return return_status(INPUT_ARGUMENT_TYPE_ERROR)
...
def KYFG_BufferAllocAndAnnounce(streamHandle, nBufferSize, pPrivate):
    ...
    kydll.KYFG_BufferAllocAndAnnounce.argtypes = (c_uint, c_ulonglong, c_void_p, c_ulonglong)
    ...
    return (return_status(KYFG_BufferAllocAndAnnounce_status),)

'''
@params:
    streamHandle - API handle to a stream. Type: int or STREAM_HANDLE
    frame - Frame index of data pointer to be retrieved. Type: int
    pAuxData KYFG_FRAME_AUX_DATA_RAW struct of auxilary data to be filled
@return FGSTATUS
    FGSTATUS - The return status of the function
'''
def KYFG_StreamGetAux(streamHandle, frame, pAuxData):
    ...
    KYFG_FRAME_AUX_DATA_RAW_C_STYLE_POINTER = POINTER(KYFG_FRAME_AUX_DATA_RAW_C_STYLE)
    kydll.KYFG_StreamGetAux.argtypes = (c_uint, c_int, KYFG_FRAME_AUX_DATA_RAW_C_STYLE_POINTER)
    kydll.KYFG_StreamGetAux.restype = c_long
    
    frame_aux_data = KYFG_FRAME_AUX_DATA_RAW_C_STYLE()
    ...
    pAuxData.reserved = frame_aux_data.reserved
