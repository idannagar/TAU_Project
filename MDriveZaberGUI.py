...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self, *args, **kwargs):
        ...
        self.refpic = FGCL.GrabIMG(self.vidobjPath, enableNUC=True)
        #--------------------------------------------------------------------------------------------------------------#
    ...
    def CENTERPushed(self):
        ...
        features = tp.locate(mat, 3,
                             topn=1,
                             preprocess=False,
                             engine='python')
        x, y, ep = features[['x', 'y', 'ep']].iloc[0].values
        dx = x - 3
        dy = -(y - 3)
    ...
    def Plot(self, imgMAT):
        ...
        topMAT, bottomMAT, centerMAT, leftMAT, rightMAT = ImPros.Divide2regions(imgMAT)
        im = ax.imshow(imgMAT, cmap='gray', vmin=3000, vmax=imgMAT.max()+500)
        ax.axis('on')
        spotsDict = ImPros.FindSpotsCenters(imgMAT)
        # mark with circles the colomators' spots
        for inds in spotsDict.values():
            circle = plt.Circle((inds[1], inds[0]), 10, edgecolor='r', fill=False)
